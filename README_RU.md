# HandARBrowser Android V14 — Galaxy A17 compatibility build

V14 специально подготовлена для проверки запуска на Samsung Galaxy A17 / Android 16 / One UI 8.0.
...
V14 содержит безопасный запуск: WindowInsets применяются только после setContentView, WebView и камера инициализируются после первого кадра UI, а ошибки подсистем переводятся в статус вместо немедленного закрытия приложения.
